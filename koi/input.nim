...
func normaliseSelection*(sel: TextSelection): TextSelection =
  if (sel.startPos < sel.endPos.int):
    TextSelection(
      ...
      endPos:   sel.endPos.Natural
    )
  else:
    TextSelection(
      startPos: sel.endPos.int,
      endPos:   sel.startPos.Natural
    )
...
proc isDoubleClick*(): bool =
  ...
  ui.mbLeftDown and
  core.getTime() - ui.lastMbLeftDownT <= DoubleClickMaxDelay and
...
proc setShortcuts*(sm: ShortcutMode) =
  ...
  for e in TextEditShortcuts: shortcuts[e] = @[]
  case sm
  of smWindows, smLinux:
    for k, v in g_textFieldEditShortcuts_WinLinux: shortcuts[k] = v
  of smMac:
    for k, v in g_textFieldEditShortcuts_Mac: shortcuts[k] = v
